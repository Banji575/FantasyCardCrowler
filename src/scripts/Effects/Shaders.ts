...
export class Shader{
    ...
    shade(){
        const shade = new Phaser.Display.BaseShader('bs', fragmentShader3)
        const shader = this.scene.add.shader(shade)
        shader.x = System.getInstance().width/2
        shader.y = System.getInstance().height/2
    }
}
